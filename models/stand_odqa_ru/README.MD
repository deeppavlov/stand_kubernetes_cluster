...
4. Install DeepPavlov:
    ...
    6. Download model components:
        ```
        python3.6 -m deeppavlov.deep download deeppavlov/configs/odqa/ru_odqa_infer_prod.json
        ```
    7. Return to the `stand_odqa_ru` dir:
        ```
        cd ..
        ```
...
4. Run Docker image:
   ```
   sudo docker run -p <host_port>:6012 --runtime=nvidia --device=/dev/nvidia<gpu_unit_int_id> -v /path/to/host/vol/map/dir:/logs stand/odqa_en
   ```