...
4. Install DeepPavlov:
    ...
    6. Download model components:
        ```
        python3.6 -m deeppavlov.deep download deeppavlov/configs/squad/squad.json
        ```
    7. Return to the `stand_squad_en` dir:
        ```
        cd ..
        ```
...
6. Specify `CUDA_VISIBLE_DEVICES` and virtual environment path (if necessary) in `run_squad_en.sh`
7. Run model:
    ```
    ./run_squad_en.sh
    ```
