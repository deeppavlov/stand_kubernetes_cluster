...
3. Create hostpath volumes for stand component and logs:
    ...
    kubectl create -f volumes/gdb_hostpath
...
db host, port, name. Add FLASK_APP=server, TG_TOKEN, WAIT_HOSTS_TIMEOUT
...
Return back agent-tg, rename base image to dream- (instead of dream_), return back dream_agent port and command.
...
docker-compose -p dream_ru -f docker-compose.yml -f assistant_dists/dream_russian/docker-compose.override.yml build ner-ru entity-detection-ru dialogpt-ru toxic-classification-ru dialogrpt-ru

ru-secret
db name ru_dream

change wiki-parser port to 9277 and entity-detection to 9203
increase spelling preprocessing timeout to 3s

## Terminus

```commandline
kubectl -n stand-demo delete secret gdb-secret
kubectl -n stand-demo create secret generic gdb-secret --from-env-file=env
kubectl create -f kuber_configs/models/graph_terminus_db/
```
